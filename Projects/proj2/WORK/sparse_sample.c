...
ROOT * addition (ROOT *M1, ROOT *M2, FILE *F)
								...
								April 14, 2026
void search (ROOT *M, int k, FILE *F)
is implemented to count the number of entries larger than k in M as an example. 
This way, we don't have to modify p2_1.c to test this function.
								April 16, 2026
*/
...
{ ENTRY *temp;
  ...
  if (M != NULL)
  { fprintf(F, "matrix at %p ...\n", M);
    for (temp = M->head; temp != NULL; temp = temp->next)
    { if (temp->val > k)
  	cnt++;
    }
    fprintf(F, "%d entries have value larger than %d.\n", cnt, k);
  }
